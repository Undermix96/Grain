...
| `frontend/nginx.conf` | The `location /api/` block uses a variable in `proxy_pass` (`$backend_upstream`) to resolve the backend hostname lazily via Docker's embedded DNS. Once a variable is used, Nginx **stops** stripping the matched location prefix from the request URI — any literal path written after the variable is sent as-is on every request, breaking every API route except that literal path. The fix is to forward `$request_uri` explicitly. This is documented, non-obvious Nginx behavior (see nginx.org tickets #803/#1067) — if this block is ever edited, re-verify this behavior rather than assuming the usual prefix-replacement rule applies. |
